...
//! (ADR-4b45f344344f, TASK-9dd22f2b0430).
//!
...
//! thing ADR-4b45f344344f has this process write into somebody else's
...
fn the_graph_carries_neither_the_dispatch_nor_the_core() {
    ...
    for forbidden in ["ank-cli", "ank-core"] {
        assert!(
            ...
            "ank-daemon links {forbidden}, and ADR-4b45f344344f forbids it: what \
...
fn the_crate_declares_a_library_and_no_binary() {
    ...
    assert!(
        ...
        "the crate declares no library target, and the verb `ank watch` links \
         one (ADR-4b45f344344f, ADR-1ea31c2f3c5a)"
    );
    assert!(
        ...
        for forbidden in ["read_to_string", "File::open", "fs::read(", "fs::write"] {
            assert!(
                ...
                "{file} calls {forbidden} inside a watched checkout: the \
                 ...
                 cache answers rather than fails (ADR-4b45f344344f)"
            );
        }
    }
    seen.sort();
    ...
    for (file, source) in sources() {
        ...
        assert!(
            ...
            "{file} parses a document, and the one document this process reads \
             ...
             is what the binary answers (ADR-4b45f344344f)"
        );
    }
}
...
/// repository that ADR-4b45f344344f did not authorise -- and the ones that are
