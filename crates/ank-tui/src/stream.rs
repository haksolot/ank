//! Following the watcher's change stream (TASK-2f7777a1fdff, ADR-4b45f344344f).
//!
...
//! what keeps ADR-4b45f344344f's "answers no verb" true with a consumer
...
impl Follower {
    ...
    fn look(&mut self) -> (bool, bool) {
        ...
        let len = meta.len();
        if len < self.offset {
            // The watcher started the file over (ADR-4b45f344344f's stream is
