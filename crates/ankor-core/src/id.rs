...
/// Identifiant canonique : `TASK-<12 hex>` ou `ADR-<12 hex>`.
/// Immuable, derive de l'acte de creation, jamais du contenu.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct EntityId {
    kind: EntityKind,
    hex: String, // 12 caracteres, minuscules
}
...
impl EntityId {
    ...
    /// Hash de l'acte de creation : timestamp, identite, titre, entropie.
    /// L'appelant fournit l'entropie (le crate ne depend d'aucun RNG).
    pub fn generate(
        kind: EntityKind,
        timestamp: &str,
        identity: &str,
...
/// Resolution par prefixe. L'ambiguite est une erreur : l'outil ne devine jamais.
pub fn resolve_prefix<'a, I>(prefix: &str, ids: I) -> Result<&'a EntityId>
where
    I: IntoIterator<Item = &'a EntityId>,
{
    // Prefixe de type optionnel en entree ("TASK-8f3a" ou "8f3a").
