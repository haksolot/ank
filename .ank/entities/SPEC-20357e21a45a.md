...
**`tui` is on that list and the binary does not answer to it yet**, which is the form this block already uses for a verb the specification has decided on before the code moves. Nothing marks the line, because the paragraph above says what marks it: `ank help` prints these verbs minus whatever is not yet implemented, and `read` sat here in exactly this state until the landing that shipped it.
