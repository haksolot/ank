...
scope:
  ...
  - install.sh
  - install.ps1
references: [SPEC-93531977642f, SPEC-f353359663d5, ADR-64f32c74a0f9]
supersedes: SPEC-b156a5571668
ratified: 8d94ea12d5f7
verified:
  - by: claude-code/opus-5+ratify
    at: 2026-09-13T18:11:09Z
schema: 4
version: 2
---
...
**An installed binary reaches a newer release through `ank update`, by the route that placed it** (ADR-64f32c74a0f9). The verb carries no fourth channel and no installer of its own: a binary inside the npm package is updated by npm, and any other released binary by `install.sh` or `install.ps1`, run against the directory it sits in. What a channel already verifies is what the update verifies, so the three channels stay the whole of how a binary arrives, the first time and every time after.
