---
id: SPEC-ac4aad6d1edb
type: spec
slug: the-data-model
title: The data model
created: 2026-09-19T18:41:47Z
author: claude-code/opus-5+docs-audit
status: accepted
scope:
  - crates/ank-core/**
  - docs/format.md
references: [SPEC-183d297253ac, SPEC-77d99d8d1ef2, SPEC-4b79c265ccb6, ADR-e1d750884b82, ADR-a8f9c603a0e7, ADR-52bb0da2023a]
supersedes: SPEC-861d09f3f85e
ratified: eff01f1fc180
verified:
  - by: haksolot@omarchy
    at: 2026-09-20T17:19:36Z
schema: 4
version: 2
---
...
|---|---|---|
| Task | `id`, `type`, `title`, `created`, `status`, `scope`, `blocked_by`, `schema`, `version` | `slug`, `author`, `done_criteria`, `criteria_by`, `verify`, `method`, `proof`, `verified` |
...
**A task may name the method its work calls for** (ADR-a8f9c603a0e7). `method` holds one name among the sibling skills the binary carries (ADR-e1d750884b82), set by `new task --method` or `amend --method` and refused at write time when the binary carries no sibling of that name (§4). It sits after `verify` and before `proof` in the canonical order, beside the other field written when the work is planned rather than when it is finished. The field is optional and its absence means nobody designated one. It is a recommendation and nothing reads it as more: `done` never reads it, `check` never faults a task for a method that did not fire, and no verifier inspects the route, so what proves a task is still the tree and never the process that produced it. Whether the method fired is a log entry and not a field, recorded below.
...
**`method` does not earn one either, on `via`'s terms** (ADR-a8f9c603a0e7). A reader that does not know the field concludes nothing false from its absence: before the field nobody could designate a method, and "nobody designated one" is exactly what its absence means now. And `amend --method` writes it into a task that already exists, a `done` one included, so a bump would leave that verb the same two choices it would leave `attest`, a file contradicting its own header or a migration performed by a write. The field is optional at parse time and readable at any version in the range.
...
**What an entry records is a field, and it is the one case that is not a convention on the message.** `records` is absent on a work entry, which is what every entry a corpus already holds is and what `ank log` means to a reader. A value names machinery: `edit`, written by the verbs that change an entity's content outside a status transition (ADR-16813b3bcf37), and `create`, the record `new` writes at birth for an entity of any kind but `log`, carrying the version and the hash of the content it produced (ADR-52bb0da2023a). `method` is the third value beside them, written by `ank log --method <name>` and titled with the name of the sibling skill that opened under a claim (ADR-a8f9c603a0e7, ADR-e1d750884b82). The work trace and the machinery are presented apart, and the count beside the trace is the trace's.
...
**How `check` reaches the commit.** `ratified` holds the hash, not the commit — a commit cannot contain its own identifier, so a field naming it could never be written by the single commit `accept` makes (§12). The pointer is the history instead, found by the subject `accept` writes and not by the file's path: one walk of `rev-list --full-history` from `HEAD`, carrying each commit's subject and body, answers every ratification a run needs, and the commit whose subject is the `ratify <id>` of §12 is the ratification; its body carries the `constraint+scope` hash. Keyed on the subject, the walk finds a ratification made while the file sat elsewhere, before the flat layout, without knowing where; made once, it costs one process where a search per entity cost 58 `rev-list` and 113 `cat-file` starts for thirty ratified decisions, a count that grew with the corpus (TASK-1b3d7b61dc8f). A ratification the walk did not see, one committed after it ran, falls back to the search over the entity's own paths. `--full-history` is there because the target is one specific commit, and history simplification dropping it would lose the anchor; it is also why the subject has to reach the default branch whole, which a merge commit keeps and a squash rewrites. Comparing against *that* hash is what makes the freeze verifiable at all: the copy in the file is written by whoever writes the file, whereas replacing the one in the commit means producing another signed commit, which is what a ratification key is for.
