---
id: SPEC-219033e25653
type: spec
slug: the-cli-surface
title: The CLI surface
created: 2026-09-14T15:03:23Z
author: claude-code/opus-5+cold-rebuild
status: accepted
scope:
  - crates/ank-cli/**
references: [SPEC-89070ce7f3b8, SPEC-e258796162c4, SPEC-77d99d8d1ef2, SPEC-a1234da5449a, ADR-e1d750884b82, ADR-a8f9c603a0e7, ADR-52bb0da2023a, ADR-64f32c74a0f9, ADR-467ce7e9cda1, ADR-4b45f344344f]
supersedes: SPEC-77689b90b211
ratified: 34b48f79dc64
verified:
  - by: haksolot@vmi3223161
    at: 2026-09-14T17:04:18Z
schema: 4
version: 3
---
...
archive   moves what is cold into .ank/archive/entities/, and commits nothing
```
...
ank log --method <name>     (the entry a sibling skill writes after the claim: records method, titled with the name)
...
ank find <query>            [--type task|adr|spec|log] [--status ...] [--scope <path>] [--free] [--all]
ank status                  [--remote]
ank review [<path>]
ank accept <id>
ank read <id>
...
ank attest <id> --compact --detached   (one entry per fact in a proof ref written before that rule)
...
ank archive                 [--dry-run]   (what is cold moves to .ank/archive/entities/; nothing is committed)
ank config <key> [<value>]  [--unset] [--user]   (a value writes; the key alone reads)
ank init [<path>]           [--at <path>]      (§9)
ank skills                  [--install]   (the skills the binary carries; --install hands them to npx skills add)
ank update                  [--check] [--version <v>]   (replaces this binary through the route that placed it; --check installs nothing)
ank help [<verb>]           (§9)
...
**`archive` is on that list and the binary does not answer to it yet**, which is the earlier state `read` and `tui` each sat in: this document declares it, `NOT_YET_DISPATCHED` in `crates/ank-cli/tests/skill.rs` names it with the task that closes the gap (TASK-97fd1992567a), and the landing that dispatches it removes that line in the same commit. Its implementation was written and measured before this declaration (LOG-8be05dd757f0), and it was held back for exactly the reason the next paragraph gives.
...
**`ank watch` is the watcher, and it answers no verb** (ADR-a22cd3196529). It keeps the corpora declared to it warm, so the `ank` a reader runs finds a cache already warm and returns sooner; it mirrors `refs/ank/claims/*` into `refs/ank/watch/<remote>/claims/`, a tracking namespace of its own, and fetches nothing else, because a mirror carries only what a reader serves and nothing reads a mirrored proof (ADR-4b45f344344f); it writes nothing else into a repository; and it emits an event when a corpus it watches changes, which states what changed and never what to do about it. It takes no claim, holds none on anybody's behalf and renews none. Nothing depends on it: every verb behaves identically with it stopped, its absence is never an error, and no route makes running it a condition of using ank.
...
**`ank skills` is on that list ahead of its landing**, in the earlier state `read`, `tui`, `mcp` and `watch` each sat in, so the paragraph above saying nothing on the list is unimplemented describes the block before this revision added the line. **The binary carries every skill under `skill/`** (ADR-e1d750884b82), the contract and each sibling, frontmatter and body, read at build time from the one source; a build with no `skill/` to read carries none and says so. `skills` prints what it carries, one line each with the name, the description and the revision. `--install` writes them to a directory of its own and hands that directory to `npx skills add <dir>`, which places them for every agent it knows with no network needed and none attempted. The flag given is the consent, and the verb asks nothing. Without node on `PATH` it prints the directory it wrote and the command to run, and exits 0; an `npx` that fails is the verb failing, and its output is the reason.

**Run in a corpus, `skills` also reports three counts per sibling** (ADR-a8f9c603a0e7): designated, how many tasks name it as their `method`; fired, how many of those carry its `method` entry; and undesignated, how many of its entries fired on tasks that designated none. The rate is read from the corpus, printed to whoever asks, and sent nowhere.

**`--method` is legal on `new task`, `amend` and `log`, and on nothing else.** On `new task` and `amend` it sets the `method` field §3 declares, one name among the siblings the binary carries; a name the binary does not carry is refused at exit 7 when the task is written, naming the ones it does. On `log` it writes an entry whose `records` is `method` and whose title is the name, refused exactly where a log write is refused: it is the one instruction a sibling that executes under a claim opens with, after the claim. That entry is a record beside the `create` record `new` writes at birth and the `edit` record the verbs changing content write (ADR-52bb0da2023a), and like them it anchors nothing. `done` never reads the field, `check` never faults a task for a method that did not fire, and no verifier inspects the route: the field is a recommendation.

**`ank update` is on that list ahead of its landing too, and it delegates rather than installs** (ADR-64f32c74a0f9). It never downloads or unpacks an archive itself: it hands the install to the official route that placed the running binary, so the checksum is verified where it already is. A binary inside the npm package runs `npm install -g @haksolot/ank@<version>`, because npm owns that tree; any other released binary runs `install.sh` on Linux and macOS, or `install.ps1` on Windows, with the directory of the running executable and no questions asked, so the update lands where the old binary was. The latest version is read with `git ls-remote --tags` against the repository, and a version older than the running one is installed only when `--version` names it. `--check` reports the running and the latest version and installs nothing. It exits 0 whenever it could ask, newer release or not, because 8 stays reserved for `check`: the answer is in what it prints and, under `--json`, in `newer`, so a script branches on a field and never on a code that means a sick corpus. **Only `update` reaches the network for this question**: no other verb checks for a newer release or announces one, since a notice printed by every verb would be traffic and output every agent pays for on every run. It never installs the skills; when the release it installed carries skill revisions other than the running binary's, it names `ank skills --install`. A binary running from a cargo target directory is refused, naming `cargo build`, because no route placed it and an update would silently swap a development build for a published one.
...
`find --all` is the one flag that reaches the archive (ADR-467ce7e9cda1). A listing answers a program whole (ADR-3e6ce108edcd), and the archive is part of the whole when asked for; without the flag the archive is neither read nor listed, so a verb that walks the corpus pays nothing for what was moved out of it. Every row of `find --json` carries `archived`, a boolean: `false` on every row a listing without `--all` can return, `true` on a row read from `.ank/archive/entities/`. The document gains that field and loses, renames and retypes none, so contract 1 holds (ADR-6fd69efb629c).
...
### The archive, and the two verbs that keep the corpus light

**`ank archive` moves what is cold into `.ank/archive/entities/`** (ADR-467ce7e9cda1): every superseded document, and every entry whose subject is cold -- an entry about a superseded document, about a task done on the **default branch**, or about an entity the archive already holds, since an entry never outlives its subject in the hot corpus. A task is never moved. The directory is a second flat directory in the format of `.ank/entities/`, readable with no parser; `show`, `log` and `find --all` resolve into it, and nothing else walks it.

The move is one rename per entity through the store, so git sees each file as a rename once it is staged and the bytes do not change. **Nothing is committed** (§12): what the hot corpus is, is a human's decision, and the result lands by pull request like a ratification. `--dry-run` prints the same list of ids and moves nothing, so the list a reviewer read is the list that moves. "Done on the default branch" is read for every candidate task in one batch, so what the verb asks git does not grow with the corpus (ADR-cc65f1388a71). It exits 0 whether it moved something or found nothing cold, and 1 when the archive already holds an entity it would move, because a move never chooses between two copies. `archive --json` answers `moved`, an array of ids, and `dry_run`, a boolean.

**`attest <id> --compact --detached` rewrites a proof ref written before a ref grew with facts rather than with runs** (ADR-4b45f344344f) to one entry per fact, adds nothing, and pushes that one ref. It needs `--detached`, which is what says a ref is being written, and takes no `--proof`, since it adds nothing; either mistake exits 1 naming the command. It exits 4 when an attestation reached the ref while it was being compacted, and 9 when the remote cannot be reached, because the ref is the whole product. `attest --json` answers `task`, `detached_proofs`, the entries kept, and `removed`, the entries dropped.
...
- **a hot entity that is cold** (ADR-467ce7e9cda1): a superseded document, or an entry about a cold subject, **reported once for the corpus** as a signal naming `ank archive`, which is the verb that moves them;
- **an archived file whose bytes no longer match the digest the index holds**: a fault. An archived entity is immutable, so matching its digest is the whole of its verification, and it is never parsed; a reference, a supersession, a blocker, an entry's subject or an identifier in prose that names an archived entity resolves, and is never reported as naming nothing;
...
**A scope naming an entity file is confronted with both roots** (ADR-467ce7e9cda1). A scope naming `.ank/entities/<ID>.md` names an entity, and an entity the archive holds is still there: the task that wrote a document and scoped itself to its file is not reported dead the day the document is archived. What exists in neither root is still dead.
