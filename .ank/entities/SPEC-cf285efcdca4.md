...
read for one window, the archive as a second root read on demand, atomic writes
and the index, the weight a corpus declares, how an invocation resolves which
corpus it is in, and the three levels of search.

## What this revision changes

It supersedes SPEC-aa4b4f119929 on three points, and on nothing else: every
other paragraph is carried forward as that document wrote it. Each point is a
decision already ratified and a binary that already obeys it; this document is
written after both, so that it describes a binary and not a plan.

- **The archive.** `.ank/archive/entities/` is a second fixed, flat root, read
  only when a verb is asked for it, with the cold rule that decides what moves
  there and `ank archive`, the verb that moves it (ADR-467ce7e9cda1).
- **The index lifecycle.** A stat is taken before a hash and may only ever say
  unchanged, closed on its one blind spot by git's racy rule
  (ADR-1556aaffe0c5). The previous text said `check` reindexes fully; what
  `check` does is stated below as the binary does it.
- **The weight budget.** `config.yml` declares how many hot files and how many
  bytes of coordination records a corpus may grow to, and `check` signals past
  either (ADR-467ce7e9cda1).
...
- **The data model** — ids, the kind registry, and the entities this layout
  stores.
- **The CLI surface** — `migrate` and `archive`, the verbs this document names
  as the commands a finding points to, and `find --all`, the one flag that
  reaches the archive.
- **ADR-467ce7e9cda1** — the archive directory, what is cold, the digest an
  archived file is held to, and the weight budget.
- **ADR-1556aaffe0c5** — freshness decided by stat before hash, and the racy
  rule.
...
### The archive: a second fixed root, read on demand

**`.ank/archive/entities/<ID>.md` is a second fixed, flat directory in the same format as `.ank/entities/`** (ADR-467ce7e9cda1): one file per entity, the file name is the id, and the file is readable with no parser and no tool. It is fixed and not configured for the reason the layout above is, and it is a directory rather than a packfile or a walk of git history because either of those breaks the promise this section exists to keep: a packfile needs a parser to find a file, and history needs git, which a reader of the format is not required to have. The store pays for it what it already pays for the previous layout: two roots, the canonical one first.

**What is cold, and nothing else.** A superseded document is cold. A log entry is cold with its subject: an entry about a superseded document, about a task done on the default branch, or about an entity the archive already holds. **A task is never cold**: a done task is the record `check`, `attest` and the proof refs point at, and the freeze of its criterion is what `done` is anchored to, so moving it would move the thing every anchor names. Its trace can go, because the trace is read by whoever asks and by nothing that runs on every verb. An entry never outlives its subject in the hot corpus: an entry copied its subject's scope when it was written and answers `ank log <id>` beside it, and leaving it hot while its subject is not puts the trace on the path every verb walks and turns the copied scope into a dead one.

**Read on demand and never by default.** The readers every verb shares, the listing, the prefix resolution and the load of the hot corpus, answer exactly what they answered before the archive existed, so no verb that writes can land on an archived entity by accident. The index does not walk the archive unless it is opened for it. Three verbs are the door: `ank show <id>` and `ank log <id>` resolve an id into both roots, and `ank find --all` lists the archive beside the hot corpus, each archived row marked (`(archived)` in text, `archived: true` under `--json`); without `--all` the archive is neither read nor listed. Moving a file into it therefore changes what a verb costs without changing what `show` answers. An id present in both roots is one entity, read hot.

**Resolved against both roots wherever an id is resolved, `check` included.** A reference, a supersession, a blocker and the subject of an entry that name an archived entity are never reported as naming nothing, and the entries that account for a hot entity (its creation record, the discrepancies recorded against its criterion) are read from both roots. `check` learns what the archive holds from its file names and never from its content. A scope naming `.ank/entities/<ID>.md` for an entity since archived is not a dead scope: the entity is still there, one directory over, and each archived file is confronted with scopes at the address it had hot. A resolution that read only the hot corpus would report the archive as nonexistence, which is the one answer worse than a slow one.

**An archived file is immutable, and verified by digest.** It is never parsed again once archived: the index reads it once when it arrives, and the content hash it records then is the file's digest and is never updated afterwards. `check` reads every archived file in full and compares its hash with that digest; a file whose bytes no longer match is **a fault**, naming `git checkout -- .ank/archive/entities/<ID>.md` to restore it, because an archived entity is never edited. Until it is restored, every open that asks for the archive hashes that file again rather than taking the edit in.

**The command that moves it is `ank archive`.** It computes the cold set by the rule above, moves each file from `.ank/entities/` to `.ank/archive/entities/` with its bytes unchanged, and `--dry-run` prints the same list and moves nothing. It writes files and never commits (§12): the caller runs `git add -A .ank`, commits, and lands the move by pull request, because deciding what the hot corpus holds is a human reviewing a diff, as a ratification is. A second run finds nothing cold. It refuses an entity the archive already holds (code 1), because a move never chooses between two copies. A hot entity that is cold by the rule is a `check` finding naming `ank archive` — **a signal and not a fault**, for the reason every layout finding is one: the corpus still answers every verb.
...
**Index lifecycle, fixed** (formerly open point 1): the index stores a content hash per entity file, and **the hash stays the record of freshness**. Every command that opens the index compares the files on disk against those hashes and incrementally reindexes what diverged — the index is therefore always up to date *at read time*, with no daemon and no watcher. The walk covers the hot corpus and the previous layout, and the archive only when the verb was asked for it. An index that is absent or of an unknown schema is rebuilt silently: deleting it is always a safe operation.

**A stat is taken before the hash, and may only say unchanged** (ADR-1556aaffe0c5). Beside each hash the index records the file's mtime, size and inode, and beside the rows it records the instant of its own last write. On open, a file is **not read** when all three fields are known on both sides and equal to the row, and its mtime is **strictly older** than that last write: the hash the row holds is its hash. Anything else — a field that differs, a field either side cannot state, no last write recorded, an mtime at or after it — reads and hashes the file exactly as a stat-less index would. A file whose hash matches and whose stat moved has its stat recorded again, so the next open can let the stat vouch rather than hash it on every one. The stat is taken before the bytes are read, so that the stat recorded is never newer than the bytes hashed. Without this every verb reads and hashes every file on every open, a cost linear in the corpus and paid by a verb that answers about one task.

**The racy rule is git's, for git's reason.** A rewrite in place, to the same size, inside one tick of a coarse clock leaves every field of the stat equal, and the only thing that betrays it is that the index cannot have seen the file after its own write. So a file whose mtime is not strictly older than the index's last write is one the index cannot vouch for, and it is hashed. The last write is **read off `index.db` itself**, from the clock of the filesystem the corpus is on and never from the wall clock of the process, since the comparison is between two timestamps that filesystem assigned. Recording it is itself a write that moves the file's mtime past the value recorded, and that is the safe direction: an earlier last write makes fewer files strictly older than it, which means more hashing and never a stat vouching for a file it should not; a recording that fails leaves the previous, earlier value. A coarse mtime widens the racy window and an unavailable field is a mismatch — on Windows the inode is the file index NTFS assigns, read through the handle — and both fall back to the hash: more hashing, never a stale answer.

**`check` takes no stat shortcut, because its answer is the corpus.** It reads and parses every hot entity file itself, through the store rather than through the index's rows, and it hashes every archived file in full against its digest. It opens the index for what it may not parse, the digests and the entries of the archive, and that open is an ordinary one. ADR-1556aaffe0c5 words this as `check` reindexing fully; what the binary does is read the files outright, which is the same guarantee — no finding of `check` ever rests on a stat — reached without rebuilding a cache it does not read from.

### Weight

**A corpus declares the weight it may grow to, and `check` says when it has** (ADR-467ce7e9cda1). `config.yml` carries `weight: {hot_files, plane_bytes}`, read and written with `ank config weight.hot_files` and `ank config weight.plane_bytes`. A counter left out takes its default, 3000 files and 4000000 bytes, exactly as an absent `context_budget` does, so a corpus that predates the key is still weighed.

- **`hot_files`** counts the entity files under `.ank/entities/`. The previous layout is not counted, since it has a signal of its own and counting it here would report one file location twice, and the archive is not counted, since leaving the hot path is what archiving is for.
- **`plane_bytes`** counts the bytes of the claim and proof records under `refs/ank/` (§7), each object once, as the read `check` already makes of them. A watcher's mirror and the records of what a remote check saw are a copy of somebody else's plane and are not weighed here.

`check --json` carries both counts, `hot_files` and `plane_bytes`, whether or not either is over. A corpus **over** a budget — at it is not over — is **a signal and never a fault**, because a heavy corpus answers every verb correctly, only more slowly. The signal names what grew and the verb that resolves it: `ank archive` for the hot files, and `ank attest <id> --compact --detached` for the records, with the heaviest proof refs named one per line. The attention budget of §5 turned a silent overflow into a finding that names the fact the corpus outgrew, and weight gets the same treatment: bloat stops being something a reader discovers by timing a verb.
