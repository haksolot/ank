...
supersedes: SPEC-20357e21a45a
ratified: ae3f243cac86
verified:
...
**`mcp` and `watch` are on that list and the binary does not answer to them yet**, which is the form this block already uses for a verb the specification has decided on before the code moves. Nothing marks either line, because the paragraph above says what marks it: `ank help` prints these verbs minus whatever is not yet implemented, and `read` and `tui` each sat here in exactly this state until the landing that shipped it.
